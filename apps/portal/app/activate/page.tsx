...
function ActivateAccountContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.success && res.redirectUrl) {
      ...
      setError(res.message || "Failed to activate account. Token may be expired.");
    }
  };
      ...
      <div className="w-full max-w-md space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-slate-400">
            Set your password below to complete account activation and access your client workspace.
          </p>
        </div>
          ...
          {!token ? (
            <div className="text-center space-y-3 p-4 bg-red-950/40 border border-red-800 rounded-xl">
              <p className="text-xs font-mono text-red-300">
                Invalid or missing invitation link. Please request a new invitation from your administrator.
              </p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} noValidate className="space-y-4">
              <div>
                <label htmlFor="activate-password" className="block text-xs font-mono text-slate-300 mb-1.5 uppercase">
                  CREATE PASSWORD *
                </label>
                <input
              ...
              <div>
                <label htmlFor="activate-confirm" className="block text-xs font-mono text-slate-300 mb-1.5 uppercase">
                  CONFIRM PASSWORD *
                </label>
                <input
                  id="activate-confirm"
...
export default function ActivatePage() {
  return (
    <Suspense fallback={<div className="py-20 text-center text-xs font-mono text-slate-500">LOADING ACTIVATION...</div>}>
