...
export function FeedbackForm({ timestampSeconds = 84, isLocked = false }: FeedbackFormProps) {
  ...
  if (isLocked) {
    return (
      <div className="p-5 bg-slate-900/90 border border-brand-yellow/40 rounded-xl space-y-2 text-xs font-mono">
  ...
  return (
    <form onSubmit={handleSubmit} className="p-4 bg-slate-900 border border-slate-800 rounded-xl space-y-3 font-mono text-xs">
      <div className="flex justify-between items-center text-slate-400">
        ...
        <span className="text-brand-yellow">Marker: {timestampSeconds}s</span>
      </div>
      <textarea
        value={comment}
        ...
        className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-sm text-white focus:outline-none focus:border-brand-yellow font-sans"
      />
      <Button type="submit" size="sm" className="w-full bg-brand-orange hover:bg-brand-orange-dark text-white font-bold">
